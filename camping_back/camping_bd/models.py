...
class CarCamping(models.Model):
    ...
    # Водоснабжение
    water_system = models.IntegerField(
        max_length=200,
        verbose_name="Запас воды/объем бака сточной воды, л"
    )

    # Дополнительное оборудование
    awning = models.IntegerField(max_length=100, verbose_name="Маркиза механическая, м")
    bike_rack = models.IntegerField(max_length=100, verbose_name="Велобагажник(вел.)")
...
class Order(models.Model):
    ...
    tents = models.ManyToManyField(Tent, through='OrderTent', related_name='orders')
...
class OrderAdditionalService(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['order', 'service'], name="order_service_unique")
        ]
